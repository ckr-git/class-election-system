<template>
  <div class="vote">
    <el-card>
      <div slot="header">
        <span>在线投票<template v-if="election.title"> - {{ election.title }}</template></span>
      </div>

      <div v-if="!electionId" style="text-align: center; padding: 40px; color: #909399;">
        <p>请从选举列表中选择一个进行中的选举进行投票</p>
        <el-button type="primary" @click="$router.push('/elections')">查看选举列表</el-button>
      </div>
